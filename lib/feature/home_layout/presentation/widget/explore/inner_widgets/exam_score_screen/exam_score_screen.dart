...
class ExamScoreScreen extends StatefulWidget {
  ...
  List<CheckQuestionAnswer>quesion;
...
class _ExamScoreScreenState extends State<ExamScoreScreen> {
  ...
  //    CheckQuestionsRequest(answers: widget.quesion)
  //  );
  // }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => viewModel..checkQuestions(CheckQuestionsRequest(answers: widget.quesion)),
      child: Scaffold(
        appBar: AppBar(
          title: Text(
            "Exam Score",
             ...
             child: Row(
               children: [
                 CircularPercentIndicator(
                   ...
                   animation: true,
                   lineWidth: 7.0,
                   percent: double.parse(state.checkQuestions!.total!.replaceAll("%", "")) / 100,
                   backgroundColor: AppColors.kRed,
                   center: Text(
                     "${(state.checkQuestions!.total?.substring(0,2))}%",
                     style: AppFonts.font20BlackWeight500,
                   ),
                 ),
                 Expanded(
